	...
	if ( '' != $search_form_template ) {
		...
		if ( 'html5' == $format ) {
			$form = '<form role="search" method="get" action="' . esc_url( home_url( '/' ) ) . '">
				<div>
					<input type="search" class="search-field" placeholder="Buscar" value="' . get_search_query() . '" name="s" title="' . esc_attr_x( 'Search for:', 'label' ) . '" />
				    <button type="submit" value="'. esc_attr_x( 'Search', 'submit button' ) .'" />Send</button>
				</div>
			</form>';
		} else {
